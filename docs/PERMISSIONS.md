...
|---|---|
| `NONE` | `TrackItResult.Error(PERMISSION_DENIED)`. No service, no crash (EC-01). |
    ...
    private suspend fun requestBackground(activity: Activity): PermissionResult {
        ...
        return if (SDK_INT >= 30) {
            ...
            NeedsSettings(
                ...
                explain = ctx.getString(R.string.trackit_background_settings_explain),
            )
        } else {
...
The reference does the same two-stage chain (`CurrentLocationProvider.kt:401-491`) and caps retries at 3 (`:440`); TrackIt keeps the cap and adds the API-30 Settings branch, which the reference does not distinguish.
...
**The SDK shows no UI.** No dialogs, no full-screen intents, no activities. The reference fires a full-screen `CATEGORY_CALL` notification when tracking looks dead (`LocationTrackingManager.kt:452-493`); that is an application decision with real policy risk, so TrackIt emits `Error(TRACKER_DEAD)` and the host decides.
...
1. `TrackIt.init(application)` in `Application.onCreate()`.
...
3. Drive the permission ladder from an `Activity` (§3), or call `TrackIt.requestPermission()` step by step and render your own rationale.
4. Observe `TrackIt.providerState()` and surface `NeedsSettings` / `Error(TRACKER_DEAD)` in your UI.
