# TrackIt SDK — Implementation Plan
...
**Package namespace:** `com.devstree.trackit` · **Repo:** `~/Work/studio/trackit` (separate project, no dependency on the source app) · **Status:** plan, not started.
...
- **`hasSpeed` / `hasBearing` default to `true`** ([A8](SOURCE-AUDIT.md)), so any construction path that forgets them produces a point claiming hardware it never had — silently disabling the network-fix rejection, the main defence against WiFi-positioning teleports. TrackIt defaults them `false`.
...
1. **No algorithm above `trackit-geo`.** A Konsist rule fails the build if `trackit-core` contains a numeric literal inside a decision expression, or imports `kotlin.math` outside `provider/FixMapper`. Every constant lives in one `TrackItConstants` data class.
...
```
                          ...
                          │  TrackIt API · FixIngestor  │
                          │  FusedLocation · FGS        │
                          │  WorkManager · Watchdog     │
                          │  ActivityTransition · Room  │
                          │  Permissions · TrackItJava  │
                          └────────┬───────────┬────────┘
                    ┌──────────────▼───┐  ┌────▼─────────┐
...
| Reference source | → TrackIt | Action |
|---|---|---|
...
| `LocationUtil.isKalmanFilteredLocation` (`:172-669`) + constants (`:69-109`) | `geo/filter/AcceptancePipeline.kt`, `TrackItConstants.kt` | Port stage-for-stage, order preserved. Signature becomes `(fix, past, state) -> (verdict, newState, point?)`. Negative-Δt branch removed (unreachable under a monotonic clock). Gen-1 constants at `:51-61` **not** ported ([A18](SOURCE-AUDIT.md)). |
...
| `utility/location/LocationTrackingManager.kt` | `core/TrackIt.kt`, `core/session/SessionManager.kt`, `core/work/Watchdog.kt` | **Rewrite.** Entirely company-coupled. Keep the watchdog logic in `isLocationHeartBeatActive()` (`:341-395`) and the raw-fix liveness clock. Drop the full-screen-intent nudge — the SDK emits an event, the host owns UI. |
...
│  └─ main/kotlin/com/devstree/trackit/geo/
│     ├─ model/   TrackFix, TrackPoint, FilterState, Verdict, FixDecision
│     ├─ filter/  KalmanFilter, AcceptancePipeline, Validation, TrackItConstants
...
│  └─ main/kotlin/com/devstree/trackit/
│     ├─ TrackIt.kt · TrackItConfig.kt          # public surface
│     ├─ domain/     model/ (TrackSession, TrackItEvent, ProviderState, TrackItResult)
│     │              repository/ (interfaces only — no Android types)
│     │              usecase/ (StartTracking, StopTracking, ResolveConfig)
│     ├─ data/       db/ (entities, daos, TrackItDatabase, mappers)
...
│     └─ di/         TrackItModule, TrackItBindings, RepositoryModule
├─ trackit-maps/ · trackit-sync/
