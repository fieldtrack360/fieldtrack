...
- **Part A — spec conformance.** [reference/capture-and-plotting-spec.md](reference/capture-and-plotting-spec.md) describes behaviour Tracker set out to reproduce. What of it is not reproduced?
- **Part B — self-conformance.** Tracker's own public surface promises things. What does it promise and not do?
...
| [G-23](#g-23) | API | MISSING | `TrackerState.motionState` never updates |
...
| [G-26](#g-26) | Build | MISSING | `fieldtrack-bridge` does not exist; the React Native package cannot compile |
...
| [G-31](#g-31) | Publishing | EXPOSURE | `TrackerConstants` is `public`, so its defaults *are* the shipped tuning table |
...
The storage for this is complete. `ActivitySegmentEntity` (`data/db/Entities.kt:233-240`), `ActivitySegmentDao` with `insert` / `openSegment()` / `close(id, endTimeMs)` / `autoCloseStale(cutoffMs, nowMs)` / `range(from, to)` (`data/db/Daos.kt:178-195`), and a graph binding (`di/TrackerGraph.kt:135`).
...
`SyncConfig.autoSync` (`fieldtrack-sync/.../TrackerSync.kt:26`) — documented as "upload as points arrive" — is read nowhere. Repo-wide, the only references to `autoSync` are its own declaration and KDoc.

Nothing outside `TrackerSync` itself calls `requestSync()` or `syncNow()`. **A host that configures sync and never calls `syncNow()` accumulates rows forever.** The queue, batching, backoff and 401 teardown all work; nothing pulls the trigger.
...
`ActivityTransitionReceiver.kt:38` emits `TrackerEvent.ActivityChange(activity, confidence = 0)` — the literal is hardcoded. No transition is filtered on confidence. `MotionConfig.activityConfidenceMin = 75` and `snapshotConfidenceMin = 50` are declared and unread.
...
This is arguably correct for Tracker: the spec's wipe exists because the reference uploaded everything and treated the device as a staging buffer, whereas Tracker is offline-first and the device is the record. **But the divergence is undocumented**, `TrackerSync.tearDown()`'s own comment claims the clear exists "so one user's positions don't leak into the next login", and that is not what the call does. Either implement the wipe or correct the claim — the present state is the one combination that is indefensible.
...
Tracker stores `movementStatus` as its own typed column and composes the string only when building a `SyncPoint`. Cleaner as a schema — a parsed enum beats a string that has to be split — but it means the value is not recoverable from a stored point, and a host round-tripping through the sync payload gets a field the local DB cannot reproduce.
...
Two different numbers, both defensible. The spec's rule discards short and rapid legs as jitter; Tracker's counts them and relies on the acceptance pipeline having already removed the jitter at capture. Tracker's is probably the better answer given its filter, and it is not the specified one, and nothing records the choice.
...
- §11.3 fetch shape · §14 fetch flow · §15 multi-device attribution — Tracker is single-user and offline-first; there is no server to fetch from
...
Gaps against Tracker's own public surface. Every one of these is a host-visible affordance that does nothing.

### G-19 — The foreground-service notification cannot be configured {#g-19}

**Verdict: MISSING.** `ServiceConfig` declares `notificationTitle`, `notificationText`, `notificationChannelId`, `notificationChannelName` and `notificationSmallIconResName` (`TrackerConfig.kt:517-521`), with builder setters `notification(title, text)`, `notificationChannel(id, name)` and `notificationSmallIconResName(...)` (`:337-343`).
...
`MotionConfig.heartbeatIntervalSec` is validated but never applied — the heartbeat cadence is `TrackerConstants.heartbeatSec`.

Worth separating: `stopTimeoutMin` and `motionTriggerDelayMs` are not merely unread, they are **shadowed**. `MotionStateMachine` takes both as constructor parameters with sensible defaults (300 000 ms and 0), and `TrackerGraph.kt:124` constructs it with **no arguments**. A host setting `stopTimeoutMin = 2` gets 5 minutes and no error.

### G-23 — `TrackerState.motionState` never updates {#g-23}

**Verdict: MISSING.** `TrackerState` (`domain/model/TrackSession.kt:112`) declares `motionState: MotionState = STOPPED`. `Tracker` updates `isReady`, `isTracking`, `currentSessionId` and `providerState`, never this. A host polling `trackIt.state.value.motionState` reads `STOPPED` for the entire life of the process.

Motion *is* observable, via `TrackerEvent.MotionChange` — but §3.13/EC-113's own guidance is "re-read authoritative state via `Tracker.state` on resubscribe; native state is always the source of truth, never a replayed event buffer". For motion state that guidance is currently wrong.
...
### G-26 — `fieldtrack-bridge` does not exist {#g-26}

**Verdict: MISSING — blocking.** `settings.gradle.kts:36` includes `:fieldtrack-bridge`. `gradle/publish.gradle.kts` carries a POM description for it. The README lists it as a shipped module. **It has never existed in git history** (`git log --all --diff-filter=A -- 'fieldtrack-bridge/*'` returns nothing).
...
- `docs/CROSS-PLATFORM.md`, cited from the README, PLAN §0 and `TrackerGraph`'s KDoc, is absent.
...
Test dependencies are declared in every module. Named test classes are cited as evidence throughout the docs — `TrackerGraphTest` (PLAN §0, `TrackerGraph` KDoc), `MixedModeTraceTest` (PLAN §9, EDGE-CASES acceptance criterion 3a), `GoldenWireTest` (`TrackerModule` KDoc) — and none of them exists.
...
### G-31 — `TrackerConstants` is `public`, and its defaults are the product {#g-31}

**Verdict: EXPOSURE.**

`TrackerConstants.kt:18` — `public data class TrackerConstants(...)` with a default for every one of ~60 parameters.
...
Blocked in practice by G-29 and G-36: `internal` in Kotlin compiles to public bytecode with mangled *function* names only, and it does not span Gradle modules — `fieldtrack-core` needs `TrackerConstants` and is a different module, so it must stay `public` today.
...
What ships: `TrackerModule.kt` (284 lines), `TrackerPackage.kt`, `TrackerSpec.kt`. Every RN consumer reads them.
...
All of it is public API. The engine is behind `fieldtrack-bridge`. **Accept and record it** — the point of writing it down is that it is now a decision rather than an assumption, and a future contributor cannot quietly relocate logic into the JS layer without noticing they are publishing it.
...
3. **[G-26](#g-26)** — `fieldtrack-bridge`. Two documented deliverables (Java client, RN package) are blocked on one absent module.
