...
import com.devstree.trackit.geo.motion.MotionEvent
 ...
 * shape as [com.devstree.trackit.capture.FixIngestor], so there is no interleaving and
...
internal class MotionController(
    ...
    fun start(config: TrackItConfig) {
        this.config = config
    ...
    private fun handle(event: MotionEvent) {
        ...
        events.tryEmit(TrackItEvent.MotionChange(changedTo, lastPoint))
    ...
    private fun onEnterMoving(config: TrackItConfig) {
    ...
    private fun onEnterStationary(config: TrackItConfig) {
