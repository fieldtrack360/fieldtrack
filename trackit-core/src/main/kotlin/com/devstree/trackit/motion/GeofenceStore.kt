...
internal class GeofenceStore(context: Context) {
    ...
    fun canAdd(id: String, isStationaryWakeFence: Boolean): Boolean = synchronized(lock) {
        ...
        isStationaryWakeFence || registrations.any { it.geofence.id == id } ||
            registrations.count { !it.isStationaryWakeFence } < TrackItGeofence.MAX_GEOFENCES
    }
    ...
    fun record(
        ...
        timestampMs: Long,
    ): TrackItGeofenceEvent = synchronized(lock) {
        val event = TrackItGeofenceEvent(
    ...
    fun events(
        ...
        limit: Int,
        offset: Int,
    ): List<TrackItGeofenceEvent> = synchronized(lock) {
        decodeEvents()
            .asReversed()
            .asSequence()
    ...
    private fun decodeEvents(): List<TrackItGeofenceEvent> = decode(
        key = EVENTS_KEY,
        serializer = ListSerializer(TrackItGeofenceEvent.serializer()),
    )
