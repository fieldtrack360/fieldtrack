...
import com.devstree.trackit.domain.model.TrackItGeofenceEvent
import com.devstree.trackit.domain.model.TrackItEvent
import com.devstree.trackit.domain.model.TrackItResult
import com.devstree.trackit.domain.model.TrackItState
...
import com.devstree.trackit.license.LicenseGate
...
import com.devstree.trackit.geo.export.GeoJson
import com.devstree.trackit.geo.filter.ClockGuard
import com.devstree.trackit.geo.export.TrackJson
...
import com.devstree.trackit.geo.plot.Snapper
import com.devstree.trackit.geo.plot.TrackBuilder
import com.devstree.trackit.geo.plot.model.LiveTrackUpdate
...
import com.devstree.trackit.geo.port.RoadSnapProvider
import com.devstree.trackit.geo.port.SnapFix
import com.devstree.trackit.geo.port.SnapRequest
...
import com.devstree.trackit.work.PruneWorker
 ...
 *
 * Obtained with [TrackIt.getInstance]:
 *
 * ```kotlin
 * val trackIt = TrackIt.getInstance(context)
 * ```
 *
 * One instance per process, wired by hand in `di/TrackItGraph.kt`. **The SDK carries no
 ...
 * own graph: `@Provides fun trackIt(app: Application) = TrackIt.getInstance(app)`.
 *
 * Nothing here throws. Every fallible call returns [TrackItResult] with a typed
 * [ErrorCode], because an SDK that throws into a host's coroutine is a crash the host
 * cannot reasonably prevent (EC-01, EC-75).
 */
public class TrackIt internal constructor(
    ...
    private val eventSink: MutableSharedFlow<TrackItEvent>,
) {

    @Volatile
    private var sensors: DeviceSensors? = null

    private val _state = MutableStateFlow(TrackItState())
    public val state: StateFlow<TrackItState> = _state.asStateFlow()
     ...
     */
    public val events: SharedFlow<TrackItEvent> = eventSink.asSharedFlow()

    @Volatile
    private var config: TrackItConfig? = null
    ...
    public fun getGeofence(id: String = TrackItGeofence.DEFAULT_ID): TrackItGeofence? =
        stationaryFence.get(id)

    /** Snapshot of every registered fence. Android and iOS both cap this at 19. */
    public fun getGeofences(): List<TrackItGeofence> = stationaryFence.all()

    /**
     * Registers or replaces a persistent system geofence.
     */
    public suspend fun addGeofence(geofence: TrackItGeofence): TrackItResult<TrackItGeofence> {
        if (geofence.id.isBlank() || geofence.latitude !in -90.0..90.0 ||
            geofence.longitude !in -180.0..180.0 || geofence.radiusM <= 0f
        ) {
            return TrackItResult.Error(ErrorCode.INVALID_CONFIG, "Invalid geofence")
        }
        return when (stationaryFence.add(geofence)) {
            StationaryFence.AddResult.ADDED -> TrackItResult.Ok(geofence)
            StationaryFence.AddResult.LIMIT_REACHED -> TrackItResult.Error(
                ErrorCode.GEOFENCE_LIMIT_REACHED,
                "A maximum of ${TrackItGeofence.MAX_GEOFENCES} geofences may be registered",
            )
            StationaryFence.AddResult.FAILED -> TrackItResult.Error(
    ...
    public suspend fun removeGeofence(id: String = TrackItGeofence.DEFAULT_ID): TrackItResult<Boolean> {
        val removed = stationaryFence.remove(id)
        return if (removed != null) TrackItResult.Ok(removed) else {
            TrackItResult.Error(
    ...
    public suspend fun removeAllGeofences(): TrackItResult<Int> {
        val removedCount = stationaryFence.removeAll()
        return if (removedCount != null) {
            TrackItResult.Ok(removedCount)
        } else {
            TrackItResult.Error(ErrorCode.GEOFENCE_REMOVAL_FAILED, "Geofence removal failed")
        }
    }
     ...
     */
    public suspend fun ready(config: TrackItConfig = TrackItConfig()): TrackItResult<TrackItState> {
        val licenseGate = LicenseGate(context)
        when (val verdict = licenseGate.check(explicit = config.license)) {
            com.devstree.trackit.license.LicenseVerdict.Licensed,
            com.devstree.trackit.license.LicenseVerdict.Waived -> Unit
            else -> {
                val failure = licenseGate.failure(verdict)!!
                eventSink.tryEmit(TrackItEvent.Error(failure.code, failure.message))
                return TrackItResult.Error(failure.code, failure.message)
            }
        }
        ...
        if (resolved.validationErrors.isNotEmpty()) {
            ...
            eventSink.tryEmit(TrackItEvent.Error(ErrorCode.INVALID_CONFIG, message))
            return TrackItResult.Error(ErrorCode.INVALID_CONFIG, message)
        }
        ...
        if (stateSyncJob == null) {
            stateSyncJob = scope.launch {
                eventSink.collect { event ->
                    when (event) {
                        is TrackItEvent.ProviderChange ->
        ...
        sessions.current()?.let { open ->
            eventSink.tryEmit(TrackItEvent.SessionInterrupted(open))
        }
        ...
        return TrackItResult.Ok(_state.value)
    }
    ...
    public suspend fun start(tag: String? = null): TrackItResult<TrackSession> {
        val active = config
            ?: return TrackItResult.Error(ErrorCode.NOT_READY, "Call ready() before start()")
        ...
        return when (val result = startTracking(active, tag)) {
            ...
            is TrackItResult.Error -> result
        }
    }

    public suspend fun stop(): TrackItResult<TrackSession?> {
     ...
     */
    public suspend fun getCurrentLocation(): TrackItResult<TrackFix> {
        val active = config
            ?: return TrackItResult.Error(ErrorCode.NOT_READY, "Call ready() before getCurrentLocation()")
        ...
        if (!provider.gpsEnabled && !provider.networkEnabled) {
            return TrackItResult.Error(ErrorCode.LOCATION_DISABLED, "Location services are disabled")
        }
     ...
     */
    public fun offerFix(fix: com.devstree.trackit.geo.model.TrackFix) {
        ingestor.offer(fix)
    }
    ...
    public companion object {
         ...
         */
        @JvmStatic
        public fun getInstance(context: Context): TrackIt = TrackItGraph.get(context).trackIt
    }
