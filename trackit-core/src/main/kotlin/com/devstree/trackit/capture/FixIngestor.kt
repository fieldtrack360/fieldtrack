...
internal class FixIngestor(
    ...
    private var constants: TrackItConstants,
     ...
     * adopt a stale session — see [com.devstree.trackit.domain.usecase.StartTrackingUseCase].
     */
     ...
     */
    fun retune(constants: TrackItConstants) {
    ...
    private suspend fun consume(queued: Queued) {
        ...
        when (ClockGuard.classify(lastElapsedNanos, fix.elapsedRealtimeNanos, outOfOrderRun, constants)) {
            ClockGuard.Step.OUT_OF_ORDER -> {
                ...
                outOfOrderRun++
                events.emit(TrackItEvent.Diagnostic(Reasons.OUT_OF_ORDER))
                return
            }
            ...
            ClockGuard.Step.REBOOT -> {
                ...
                events.emit(TrackItEvent.Diagnostic(Reasons.REBOOT_BOUNDARY))
            }
        ...
        if (point != null) {
            ...
            events.emit(TrackItEvent.Location(point))
        } else {
            events.emit(TrackItEvent.LocationRejected(result.decision))
        }
    }
