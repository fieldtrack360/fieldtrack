...
android {
    namespace = "com.devstree.trackit"
    ...
    buildTypes {
        release {
            ...
            // into com.devstree.trackit.internal, so decompiling the artifact yields
...
dependencies {
    ...
    // No DI framework, deliberately. The graph is wired by hand in di/TrackItGraph.kt so
    // that a host needs no Gradle plugin, no `@HiltAndroidApp`, and no annotation
    // processor of its own — see TrackIt's KDoc and CROSS-PLATFORM.md B-1. KSP below is
    // Room's alone.
