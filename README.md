...
- **`fieldtrack-maps` has no tests.** It is thin by design — it consumes `Arrows.place()` and draws — but "thin" is not "verified", and nothing currently fails if it stops rendering.
...
| Module | What it is |
|---|---|
| `fieldtrack-geo` | Pure-Kotlin engine packaged as an obfuscated AAR. Kalman filter, acceptance pipeline, motion state machine, turn detection, and plotting. |
| `fieldtrack-core` | The Android library and the public SDK surface. Capture, Room storage, foreground service, permissions, workers, DI. |
| `fieldtrack-maps` | Optional. Google Maps rendering that consumes `Arrows.place()` rather than recomputing it. |
| `fieldtrack-sync` | Optional. HTTP upload with a retry queue and 401 teardown. OkHttp is `compileOnly`. |
| `fieldtrack-snap` | Optional. `OsrmSnapProvider` — map-matching against an OSRM server. Depends on `fieldtrack-geo` only. OkHttp is `compileOnly`. |
| `sample-android` | The demo app: start/stop, live map with arrows, 3-layer debug overlay, decision-log viewer. |

Dependency direction, with nothing pointing backwards: `sample-android`, `fieldtrack-sync` → `fieldtrack-core` → `fieldtrack-geo` ← `fieldtrack-maps`, `fieldtrack-snap`.

## Using it

**No DI framework, no Gradle plugin, no annotation processor.** The whole integration is one call. The graph is wired by hand in `di/TrakerGraph.kt`; an earlier revision shipped Hilt inside `fieldtrack-core` and required every host to apply the Hilt plugin and annotate its `Application` with `@HiltAndroidApp` — that was removed, and the reasoning is in [PLAN.md](docs/PLAN.md) §0.
